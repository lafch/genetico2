...
public class bAsistenciaDocente {
    ...
    private String b_sesObservacion;
    ...
    public String getB_sesObservacion() {
        return b_sesObservacion;
    }

    public void setB_sesObservacion(String b_sesObservacion) {
        this.b_sesObservacion = b_sesObservacion;
    }
    ...
    public List<AcDocente> autocomplete2( Object suggestion ) {
     ...
     public List<bAsistenciaDocente> autocompleteDiaria(Object suggestion) {
        List<bAsistenciaDocente> listaEmpleados = new ArrayList<bAsistenciaDocente>();
        String ref = (String) suggestion;
        HSAsistenciaDocenteDAO dao = (HSAsistenciaDocenteDAO) ServiceFinder.findBean("SpringHibernateDaoAsistenciaDocente");
        List empleados = dao.buscarDocente_x_dato(ref);
...
//            System.out.println(asis);
    ...
    public void guardarObservacionAsistencia(ActionEvent event) {
        ...
        try {
            if (this.getAsisDocente().getEstasis_code_diario().equals("0")) {
                this.setOncompleteADD("javascript:alert('SELECCIONE UN TIPO DE ASISTENCIA');");
            }  else {
                HSAsistenciaDocenteDAO dao = (HSAsistenciaDocenteDAO) ServiceFinder.findBean("SpringHibernateDaoAsistenciaDocente");
                AcSesionAsistencia sesAsis= new AcSesionAsistencia();
                sesAsis.setSesEstadoDocTipo(this.getAsisDocente().getEstasis_code_diario());
                estadoDocTipo=this.getAsisDocente().getEstasis_code_diario();
                String sesObservacion=this.getAsisDocente().getSesObservacion();
