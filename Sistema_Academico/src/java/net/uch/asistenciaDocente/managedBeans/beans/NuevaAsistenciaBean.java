 ...
 * @author cesardl
 */
public class NuevaAsistenciaBean {
    ...
    public SelectItem[] getN_seccion() {
        ...
        List lista = dao.listarSecciones(this.getN_especialidad_code(), sem_id.getId());
        if (lista.size() > 0) {
    ...
    public SelectItem[] getN_tipses() {
        try {
            HSCatalogoDAO catalogoDAO = (HSCatalogoDAO) ServiceFinder.findBean("SpringHibernateDaoCatalogo");
            List l = catalogoDAO.seleccionarCatalogo("047");
            n_tipses = new SelectItem[l.size() + 1];

            for (int i = 0; i < l.size(); i++) {
                TbCatalogo cat = (TbCatalogo) l.get(i);
                n_tipses[i + 1] = new SelectItem("047" + cat.getCatCodigoElemento(), cat.getCatDescripcionElemento());
            }
        } catch (Exception e) {
    ...
    public String toString() {
        return "id docente: " + this.getN_doc_id()
                + "\ntipo asis 050 " + this.getN_tipasis_code()
