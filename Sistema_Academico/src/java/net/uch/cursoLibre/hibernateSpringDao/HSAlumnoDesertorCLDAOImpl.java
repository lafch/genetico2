...
public class HSAlumnoDesertorCLDAOImpl extends HibernateDaoSupport implements HSAlumnoDesertorCLDAO {
    ...
    @Override
    public List<BeanReporte> listarDesertores( String sNomApeFiltro, String sNumTlfFiltro, int iModuloId, int iCursoId, List<String> lstActivos, List<String> lstActivos2 ) {
        int iSizeResult;
        String sQuery;
        BeanReporte bRep;
        Object obj;
        ...
        try {
            ...
            String sMotWhere2 = "";
            //para ver si hay grado de inters o no
            if ( !lstActivos2.isEmpty() ) {
                for ( String sMot2 : lstActivos2 ) {
                    if ( !"0".equals( sMot2 ) ) {
                        sMotWhere2 = " ";
                    } else {
                        sMotWhere2 = " WHERE t.grado=1";
                    }
                }
            } else {
                sMotWhere2 = " ";
            ...
            sQuery = "select * from\n"
                    + "(SELECT t.*,max(t.arb_id) max_id FROM\n"
                    ...
                    + " if((SELECT max(ad.grado_interes) FROM cl_alumno_desertor ad WHERE ad.alu_id=a.alu_id AND ad.arb_mod_id=aMod.arb_id AND ad.activo=1)is null,'0',(SELECT max(ad.grado_interes) FROM cl_alumno_desertor ad WHERE ad.alu_id=a.alu_id AND ad.arb_mod_id=aMod.arb_id AND ad.activo=1)) grado,\n "
                    + "aCur.arb_id \n"
                    + "FROM cl_alumno a\n "
            ...
            sQuery += sMotWhere2 + " \n"
                    + "GROUP BY t.alu_id\n "
                    + "ORDER BY fecha_termino desc,t.curso,t.alumno)t2 \n"
                    + "WHERE  max_id=:cur_id";
            ...
            if ( lstResult != null && !lstResult.isEmpty() ) {
                ...
                for ( int i = 0; i < iSizeResult; i++ ) {
                    ...
                    bRep.setExpr13( CommonWeb.parseObjToInt( objFila[11] ) > 0 ? "SI" : "NO" );//grado: Si es mayor a 1 que esta interesado
