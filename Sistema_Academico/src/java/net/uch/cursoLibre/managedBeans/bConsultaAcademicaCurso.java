...
//        boolean blEsProgAux = w_area_id == ConstantesWeb.ID_AREA_PROG_AUX;
    ...
    public void cambioTaller( ActionEvent event ) {
        w_sec_id = 0;
    }
    ...
    public String getW_taller() {
        return w_taller;
    }

    public void setW_taller( String w_taller ) {
        this.w_taller = w_taller;
    }
   ...
   public SelectItem[] getCboInstituciones() {
        try
        {
            ...
            if(tam>0)
            {
                                m_cboInstituciones = new SelectItem[tam + 1];
                                m_cboInstituciones[0] = new SelectItem(0, "[Seleccione]");
                for(int i = 0; i < tam; i++)
                {
                    String codigo = ((TbCatalogo)aDocumentosListados.get(i)).getCatCodigoGrupo() + ((TbCatalogo)aDocumentosListados.get(i)).getCatCodigoElemento();
                                        if ( "078004".equals( codigo ) || "078005".equals( codigo ) ) {
                                        ...
                                        m_cboInstituciones[i+1] = new SelectItem(codigo,((TbCatalogo)aDocumentosListados.get(i)).getCatDescripcionElemento());
                                        }
                    //comboInstitucion[i+1] = new SelectItem(codigo,((TbCatalogo)aDocumentosListados.get(i)).getCatDescripcionElemento());                   
                }
            }
    ...
    public SelectItem[] getCboArea() {
        ...
        if ( m_sInstId != null ) {
            ...
            if ( lstAreas != null && !lstAreas.isEmpty() ) {
                cboArea = new SelectItem[ lstAreas.size() + 1 ];
        ...
        cboArea[0] = new SelectItem( "0", "[Seleccione]" );
        return cboArea;
    }
...
//            }
    ...
    public SelectItem[] getCboTaller() {
        HSArbolAcademicoClDao arbAcadDAO;
        List<ClArbolAcademico> lstTalleres;
        if ( this.w_cur_id == 0 ) {
            cboTaller = new SelectItem[ 1 ];
        } else {
            try {
                arbAcadDAO = CommonDAO.getClArbolAcademicoDAO();
                lstTalleres = arbAcadDAO.seleccionarTalleres( this.w_cur_id );

                cboTaller = new SelectItem[ lstTalleres.size() + 1 ];
                for ( int i = 0; i < lstTalleres.size(); i++ ) {
                    cboTaller[i + 1] = new SelectItem( lstTalleres.get( i ).getArbId(), lstTalleres.get( i ).getArbDescripcion() );
                }
            } catch ( Exception e ) {
                System.err.println( "Error al cargar areas" );
            }
        }
        cboTaller[0] = new SelectItem( 0, "[Seleccione]" );
        return cboTaller;
    }

    public void setCboTaller( SelectItem[] cboTaller ) {
        this.cboTaller = cboTaller;
    }
    ...
    public SelectItem[] getCboSeccion() {
        ...
        try {
            if ( w_sed_id > 0 && w_tall_id > 0 && w_mod_id > 0 && !"".equals( w_ano_id ) && Integer.parseInt( w_ano_id ) > 0 ) {
                DateFormat format = new SimpleDateFormat( "yyyy-MM-dd" );
                SimpleDateFormat sdf = new SimpleDateFormat( "yyyy-MM-dd" );
                String TfechaI = format.format( this.secFinicio );
                String TfechaF = format.format( this.secFfin );
                Date w_fechaI = sdf.parse( TfechaI );
                Date w_fechaF = sdf.parse( TfechaF );
                lstSecciones = CommonDAO.getClArbolAcademicoDAO().listarSeccionesXTallerAnoYSede( w_tall_id, Integer.parseInt( w_ano_id ), w_sed_id, w_fechaI, w_fechaF );
            }
    ...
    public SelectItem[] getRdoOpciones() {
        ...
        rdoOpciones[2] = new SelectItem( "3", "REGISTRO [Inicio de curso]" );
        rdoOpciones[3] = new SelectItem( "4", "REGISTRO [Fin    de curso]" );
        return rdoOpciones;
    }
    ...
    public Date getSecFinicio() throws ParseException {

        if(this.secFinicio==null){
        Date date_t = new Date();
        //Calendar date = new GregorianCalendar( date_t.getYear(), date_t.getMonth() + 1, date_t.getDate() );
        mes = date_t.getMonth() - 3;
        anio = date_t.getYear() + 1900;
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
        Date desde = sdf.parse("1" + "/" + mes + "/" + anio);
        this.secFinicio=desde;
        }
    ...
    public Date getSecFfin() throws ParseException {
        if(this.secFfin==null){
        Date date_t = new Date();
        //Calendar date = new GregorianCalendar( date_t.getYear(), date_t.getMonth() + 1, date_t.getDate() );
        dia=date_t.getDay()+9;
        mes = date_t.getMonth()+1;
        anio = date_t.getYear() + 1900;
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
        Date hasta = sdf.parse(dia + "/" + mes + "/" + anio);
        this.secFfin=hasta;
        }
        System.out.println( "fecha fin"+ secFfin );
        return this.secFfin;
    }
    ...
    public void seleccionarSeccion( ActionEvent event ) throws Exception {
        ...
        if ( !isEsProgAux() ) {
            lstHorario = horarioDAO.seleccionarHorario( w_sec_id );
            w_docente = lstHorario.isEmpty() ? "No determinado" : lstHorario.get( 0 ).getAcDocente().getDocNombre();
        } else {
            lstHorario = horarioDAO.seleccionarHorarioAsignatura( w_sec_id, iIdAsignatura );
            w_docente = lstHorario.isEmpty() ? "No determinado" : lstHorario.get( 0 ).getAcDocente().getDocNombre();
        }
...
//                lstSisEvaluacionXCurso = sisEvaDAO.listarSistemaEvaluacionPorCurso( clArbApertTall.getArbapeId() );
    ...
    public JasperPrint cargar_reporteDynamic( int iIdReporte ) {
        String sNombreReporte, sNombreLocal;
        ...
        try {
            ...
            if ( iIdReporte == 3 && ( w_area_id == 5 || w_area_id == 23 || w_area_id == 1554 || w_area_id == 1710 || w_area_id == 1762 || w_area_id == 1670 || w_area_id == 1704 ) ) {
                sNombreLocal = CommonDAO.getAcDocenteDAO().LocalSeleccionado( w_sed_id );
                // hmParams.put( "p_campus", this.w_sed_id == 1 ? "OLIVOS" : "COLONIAL" );
                hmParams.put( "p_campus", sNombreLocal );
                ...
                if ( w_area_id == 5 || w_area_id == 23 || w_area_id == 1554 || w_area_id == 1710 || w_area_id == 1762 || w_area_id == 1670 || w_area_id == 1704 ) {
                    ...
                    URL FileImagen = ctx.getResource( "/Imagenes/actions/logo_p.jpg" );
                ...
                if ( isEsProgAux() ) {
                    sNombreReporte = "rep_registro_por_aula_auxiliar_CEP.jasper";
                } else {
